...
A["KIRBY2LS"]=function kirby2ls(x::AbstractVector)
    ...
    for i = 1:M
        term1 = -Y[i]
        term2 = (x[1] + x[2] * X[i] + x[3] * X[i]^2)/(x[1] + x[2] * X[i] + x[3] * X[i]^2)
...
A["COATING"]=function coating(x::AbstractVector)
    ...
    for ind = 1:(M/4)
        ...
        term2 = x[5]+ETA1[i]*x[6]+ETA2[i]*x[7]+ETA2[i]*ETA1[i]*x[8]-Y[i]
...
A["YATP2CLS"]=function yatp2cls(x::AbstractVector)
    ...
    for i = 1:N
        ...
        term3=0
        for j = 1:N
            term3 = term3-y[i]-z[j]-A-(y[i]+z[i])*cos(x[i,j])
            term1=term1+x[i,j]
            term2=term2+x[i,j]
...
A["COATINGNE"]=function coatingne(x::AbstractVector)
    ...
    for ind = 1:(M/4)
        ...
        term3 = SCALE1*x[i+8]
        term4 = SCALE2*x[i+M4+8]
        sum=sum+term11+term22+term3+term4
    end
    return sum #, grad
end
...
A["YFITU"]=function yfitu(x::AbstractVector)
    ...
    y[17]=-35.747869
    for i = 1:(p+1)
        term1 = -y[i]+x[3]*tan(x[1]*(1-1/p)+x[2]*i/p)
        sum=sum+term1^2
    end
    return sum #, grad
end
...
A["SSBRYBND"]=function ssbrybnd(x::AbstractVector)
    ...
    for i = 1:LB
        coeff = exp(6*(i-1)/(N-1))
        sum1=K1*coeff*x[i]+K1*x[i]^3
        for j = 1:(i-1)
            coeff = exp(6*(j-1)/(N-1))
            term1 = -K3*coeff*x[j]+-K3*x[j]^2
            sum1=sum1+term1
        end
        for j = (i+1):(i+UB)
            coeff = exp(6*(j-1)/(N-1))
    ...
    for i = (LB+1):(N-UB-1)
        coeff = exp(6*(i-1)/(N-1))
        sum1=K1*coeff*x[i]+K2*x[i]^2
        for j = (i-LB):(i-1)
            coeff = exp(6*(j-1)/(N-1))
            term1 = -K3*coeff*x[j]+-K3*x[j]^3
            sum1=sum1+term1
        end
        for j = (i+1):(i+UB)
            coeff = exp(6*(j-1)/(N-1))
...
A["FMINSRF2"]=function fminsrf2(x::AbstractVector)
    ...
    for i = 1:(P-1)
        ...
        for j = 1:(P-1)
            ...
            sum1=sum1+(1/SCALE)*sqrt(term1+term2+term3)
        end
    end
...
A["SPINLS"]=function spinls(x::AbstractVector)
    ...
    for i = 2:N
        term3=0
        for j= 1:(i-1)
            term3= term3-v[i,j]^2+(x[i]-x[j])^2+(y[i]-y[j])^2
            sum=sum+term3^2
        end
    end
    return sum #, grad
...
A["DJTL"]=function djtl(x::AbstractVector)
    ...
    BU2=100-x[2]
    BL2=x[2]
    sum=OBJ+10^10*CU1^2+10^10*CL1^2+10^10*CU2^2+10^10*CL2^2+10^10*BU1^2+10^10*BL1^2+10^10*BU2^2+10^10*BL2^2
    return sum #, grad
end
